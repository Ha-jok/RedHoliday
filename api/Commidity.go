...
func CommidityIntroduction (c *gin.Context){
	//获取参数
	uid := c.Param("uid")
	add := c.PostForm("add")
	...
	//提取信息
	commidity := service.QueryCommidity(uidm)
	...
	if add == "add"{
		...
		//更改数据库中的数据
		service.AddCart(uidm,username)
	}
}

//商品评论，/redholiday/commidity/:uid

func CommidityComment (c *gin.Context){
	//获取参数
